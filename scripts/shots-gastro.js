// Gastro accordion görsel doğrulaması: grouped görünümde bölüm başlığını aç/kapat.
...
(async () => {
  ...
  await page.goto('http://localhost:8097/', { waitUntil: 'networkidle0' });
  await page.waitForFunction(() => document.body.innerText.includes('Skorla'), { timeout: 15000 });
  await wait(600);

  // Grouped görünümde "GASTRO / HEPATOLOJİ" başlık metnini (tam eşleşme) bul ve ortala
  ...
  let ok = false;
  for (let i = 0; i < 6 && !ok; i++) { ok = await center(); if (!ok) { await page.evaluate(() => { const s = [...document.querySelectorAll('*')].find((e) => e.scrollHeight > e.clientHeight + 200 && e.clientHeight > 300); if (s) s.scrollTop += 700; }); await wait(300); } }
  await wait(500);
  await page.screenshot({ path: path.join(OUT, '06-gastro-closed.png') });
  console.log('closed shot, found=' + ok);

  // Başlığı tıkla (accordion'u aç)
  await page.evaluate(() => {
    const target = 'Gastro / Hepatoloji'.toLocaleUpperCase('tr');
    const el = [...document.querySelectorAll('div')].find((e) => e.textContent && e.textContent.trim() === target);
    if (el) el.click();
  ...
  await center();
  await wait(500);
  await page.screenshot({ path: path.join(OUT, '07-gastro-open.png') });
